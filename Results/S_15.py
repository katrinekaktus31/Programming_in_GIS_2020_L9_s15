...
input_shape = r"D:\python_project\PC_project\S_arcPy\S_15\Programming_in_GIS_2020_L9_s15\Results\us_cities_level_1.shp"
...
arcpy.CopyFeatures_management("citiesLayer", "us_cities_level_1" + str(popsize) + ".shp")
arcpy.AddMessage('Create feature layer ' + "us_cities_level_" + str(popsize) + ".shp")


# Reprojest raster
arcpy.ProjectRaster_management(inputraster, inputraster[-5] + "_NAD" + ".tif", cities, geographic_transform = "WGS_1984_(ITRF00)_To_NAD_1983")
arcpy.AddMessage('Reprojected raster ' + inputraster + "in to " + inputraster[-5] + "_NAD" + ".tif")

# Clip raster
clip_raster = arcpy.sa.ExtractByMask(inputraster[-5] + "_NAD" + ".tif", clipzone,)
clip_raster.save(inputraster[-5] + "_NAD_extract_by_mask" + ".tif")

# Get value temperature
with arcpy.da.SearchCursor("us_cities_level_" + str(popsize) + ".shp", "SHAPE@XY") as cursor:
    temperature = []
    for i in cursor:
        res = arcpy.GetCellValue_management(inputraster[-5] + "_NAD_extract_by_mask" + ".tif", str(i[0][0]) + " " + str(i[0][1]))
        value = float(str(res.getOutput(0)).replace(",", "."))
        temperature.append(float(value))

# Add new fields and update
arcpy.AddField_management("us_cities_level_1" + str(popsize) + ".shp", "TEMPERATUR", 'FLOAT')
arcpy.AddField_management("us_cities_level_1" + str(popsize) + ".shp", "EXCESS", 'FLOAT')
arcpy.AddMessage('Added new fields to the  TEMPERATUR and EXCESS')

with arcpy.da.UpdateCursor("us_cities_level_1" + str(popsize) + ".shp", ["TEMPERATUR", "EXCESS"]) as Upcursor:
    i = 0
    for row in Upcursor:
        row[0] = temperature[i]
